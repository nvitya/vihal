...
void THwUsbEndpoint_atsam_v2::SendAck()
{
	...
	regs->EPSTATUSCLR.reg = USB_DEVICE_EPSTATUS_STALLRQ1;
  regs->EPSTATUSSET.reg = (USB_DEVICE_EPSTATUS_DTGLIN | USB_DEVICE_EPSTATUS_BK1RDY);  // the ACK must be sent with DATA1
}
...
void THwUsbEndpoint_atsam_v2::Stall()
{
	if (iscontrol || !dir_htod)
	{
		regs->EPSTATUSSET.reg = USB_DEVICE_EPSTATUS_STALLRQ1;
	}
	else
	{
		regs->EPSTATUSSET.reg = USB_DEVICE_EPSTATUS_STALLRQ0;
	}
}
