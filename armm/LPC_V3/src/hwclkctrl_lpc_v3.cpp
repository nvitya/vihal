...
#ifdef SYSCON_FLASHCFG_FLASHTIM_MASK
...
{
  // THwClkCtrl_lpc_v3::SetupPlls(bool aextosc, unsigned abasespeed, unsigned acpuspeed)
  ...
  else
  {
    ...
    return true; // using internal clock is tno implemented
  }
  ...
  SYSCON->PDRUNCFGSET[0] = (1 << 22);   // Power off PLL during setup changes
	...
	unsigned pllmul = target_speed_hz / basespeed;
