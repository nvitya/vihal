...
HW_GPIO_REGS * THwPinCtrl_rp::GetGpioRegs(int apinnum)
{
...
bool THwPinCtrl_rp::PinSetup(int aportnum, int apinnum, unsigned flags)
{
  if ((apinnum < 0) || (apinnum >= MAX_GPIOPIN))
  {
    return false;
...
void THwPinCtrl_rp::GpioSet(int aportnum, int apinnum, int value)
{
  uint32_t pinmask = (1 << apinnum);
...
void THwPinCtrl_rp::GpioIrqSetup(int aportnum, int apinnum, int amode)
{
...
void TGpioPort_rp::Assign(int aportnum)
{
	regs = sio_hw;
}

void TGpioPort_rp::Set(unsigned value)
{
  regs->gpio_out = value;
}
...
void TGpioPin_rp::Assign(int aportnum, int apinnum, bool ainvert)
{
	portnum = aportnum;
  pinnum = apinnum;
  inverted = ainvert;
...
void TGpioPin_rp::SwitchDirection(int adirection)
{
	if (adirection)
	{
